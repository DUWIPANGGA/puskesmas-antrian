...
<head>
    ...
    <title>Display Pemeriksaan | Puskesmas Jagapura</title>
...
<body class="p-6">
    <div class="h-full flex flex-col gap-6">
        ...
        <header class="glass rounded-[2rem] px-10 py-6 flex justify-between items-center shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-black text-[#d81b60] tracking-tight">DISPLAY PEMERIKSAAN</h1>
            ...
            <div class="flex items-center gap-8">
                ...
                @if($selectedPoliId)
                    <a href="{{ route('admin.display.pemeriksaan') }}" class="flex items-center gap-2 px-6 py-3 bg-pink-50 hover:bg-pink-100 rounded-full transition font-bold text-[#d81b60]">
        ...
        <main id="display-container" class="flex-1 grid gap-6 {{ $selectedPoliId ? 'grid-cols-12' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' }}">
            @if($selectedPoliId)
                ...
                <div id="focus-poli" class="col-span-12 lg:col-span-8 glass rounded-[3rem] p-12 flex flex-col justify-center items-center text-center shadow-xl border-4 border-pink-500/10 relative overflow-hidden">
                    ...
                    <div class="flex flex-col items-center">
                        <h2 class="text-slate-400 font-medium text-lg mb-2 uppercase tracking-widest">Antrian Sekarang</h2>
                        ...
                        <h3 id="focus-pasien-name" class="text-5xl font-black text-[#d81b60] mt-4 uppercase tracking-wider">—</h3>
                    </div>
                    
                    <div class="mt-12 py-5 px-12 bg-slate-900 text-white rounded-[2rem] shadow-2xl flex items-center gap-4">
                        <span class="material-symbols-outlined text-[#f48fb1] text-3xl">medical_information</span>
                        <p id="focus-doctor" class="text-2xl font-bold tracking-tight">Dokter sedang bersiap</p>
                    </div>
                </div>
            ...
            @else
                {{-- Grid view will be populated by JS --}}
            @endif
        </main>

        <footer class="h-14 shrink-0 bg-slate-900 rounded-[1.5rem] overflow-hidden flex items-center shadow-inner">
            <div class="bg-[#d81b60] px-6 h-full flex items-center">
                <span class="text-white font-black text-sm uppercase tracking-widest">PEMERIKSAAN</span>
            </div>
            <div class="flex-1 px-6 text-pink-100/50 font-bold overflow-hidden">
                <marquee scrollamount="5" class="pt-1">
                    Silakan Menuju Ruang Pemeriksaan Saat Nama Anda Dipanggil • Puskesmas Jagapura Melayani Dengan Sepenuh Hati • Jagalah Kebersihan Selama Berada di Area Puskesmas
                </marquee>
            </div>
        </footer>
    </div>
    ...
    <script>
        ...
        function speakQueue(nomorAntrian, poliName, pasienName) {
            if (!('speechSynthesis' in window)) return;
            const synth = window.speechSynthesis;
            synth.cancel();
            const formattedNomor = nomorAntrian.split('').join(' ');
            const message = `Nomor antrian... ${formattedNomor}... atas nama... ${pasienName}... silakan menuju... ${poliName}`;
        ...
        function updateDisplay() {
            ...
            fetch(url).then(res => res.json()).then(data => {
                ...
                data.polis.forEach(p => {
                    if (p.current_number !== '-') {
                        const pre = lastState[p.id];
                        if (!pre || pre.number !== p.current_number || pre.updated !== p.current_update) {
                            speakQueue(p.current_number, p.name, p.current_name);
                        }
                    }
                    lastState[p.id] = { number: p.current_number, updated: p.current_update };
                });
                ...
                if (selectedPoliId) {
                    ...
                    if (p) {
                        ...
                        const finalNum = p.current_number === '-' ? '—' : p.current_number;
                        if (numEl.innerText !== finalNum) {
                            numEl.innerText = finalNum;
                            if (finalNum !== '—') {
                                document.getElementById('focus-number-container').classList.add('animate-new');
                                setTimeout(() => document.getElementById('focus-number-container').classList.remove('animate-new'), 10000);
                            }
                        }
                        const nameEl = document.getElementById('focus-pasien-name');
                        if(nameEl) nameEl.innerText = p.current_name || '—';
                        document.getElementById('focus-doctor').innerText = p.doctor;
                    ...
                    data.polis.forEach(p => {
                        html += `
                            <div onclick="window.location.href='?poli_id=${p.id}'" class="glass rounded-[2rem] p-8 flex flex-col justify-between shadow-sm hover:shadow-xl transition-all cursor-pointer border-l-8 border-[#d81b60]">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <p class="text-[#d81b60] font-black text-[10px] tracking-widest uppercase">${p.code || 'KLINIK'}</p>
                                        <h3 class="text-xl font-extrabold text-slate-800">${p.name}</h3>
                                    </div>
                                ...
                                <div class="flex-1 flex flex-col justify-center items-center py-6">
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">DIPANGGIL</p>
                                    <div class="text-7xl font-black text-slate-950 tracking-tighter">${p.current_number === '-' ? '—' : p.current_number}</div>
                                    <p class="text-xs font-bold text-[#d81b60] mt-2 uppercase">${p.current_name || ''}</p>
                                </div>
                                <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
                                    <p class="text-xs font-bold text-slate-400 italic">${p.doctor}</p>
                                    <span class="material-symbols-outlined text-pink-200">chevron_right</span>
                                </div>
                            </div>`;
                    });
        ...
        setInterval(() => {
            ...
            const el = document.getElementById('live-clock'); if (el) el.innerText = `${h}:${m}:${s}`;
        }, 1000);
    </script>
</body>
</html>
